<template>
  <!-- 我是播放器 -->
  <div class="myBofnagqi" :class="UpAndDown ? 'up' : 'down'">
    <!-- 隐藏和显示 -->
    <div class="show" @click="changeUpAndDown" style="cursor: pointer">
      <Icon
        icon="bx:down-arrow"
        width="30"
        height="30"
        :class="{ icondao: !UpAndDown }"
      />
    </div>
    <!-- 播放器 -->
    <audio
        ...
        <div class="aax">
          ...
          <div class="myBofnagqi-bofangqianniu-wai">
            ...
            <div class="myBofnagqi-bofangqianniu-nei-you">
              <!-- 是否添加到我喜欢 -->
              <el-button type="primary" class="like" circle>
                <Icon
              ...
              >
                <Icon
                  :icon="PlaybackMode.arr[PlaybackMode.index]"
                  width="1.5vw"
                  height="1.5vw"
                ></Icon>
              </el-button>
...
import { reqSongUrl, reqSongDetail, reqLyric } from "@/api/songListOrSong";
...
// 播放器升降
let UpAndDown = ref(true);
// 隐藏播放器
emitter.on("showUpAndDown", (pd = !UpAndDown.value) => {
  // 默认取反，可传值
  UpAndDown.value = pd;
});
...
function changeUpAndDown() {
  UpAndDown.value = !UpAndDown.value;
}
...
let PlaybackMode = reactive({
  index: 0,
  // 顺序 循环 随机
  arr: ["subway:random", "cil:loop-1", "el:random"],
});
// 改变播放类型
...
async function changeSong() {
  ...
  if (data.code == -460) {
    ...
    console.log(
      "api的音乐url有问题.九万里风鹏正举,风休住，蓬舟吹取三山去。",
      data
    );
  } else {
    ...
    .myBofnagqi-bofangqianniu-you {
      ...
      .aax {
        ...
        .myBofnagqi-bofangqianniu-wai {
          ...
          padding: 2px 10px;
          border-radius: 10px;
          box-shadow: 0.1vw 0.1vw 0.8vw #585858;
